...
Three statues of Finnish authors stand among the lime trees of Helsinki's central park, each facing the same direction. Players walk the park from west to east, solving puzzles tied to the statues, the benches and a small pool at the eastern end. The hidden message: a single word the writers leave behind for every generation. → espa.epicscape.game
Rikhardinkatu Library, Helsinki — The Panda's Tooth Mystery
The library's main hall is home to a wooden panda that has lost a tooth — and with it, the library has fallen into chaos. The children's section, the archives, the elevator and the upper floors: each holds a clue. Players work through three mini-tracks before facing the great puzzle that restores order. The library is Epic Scape's longest-running partnership: two years, three contract renewals. → riku.epicscape.game
...
Epic Scape runs entirely in the mobile browser. No app installation required. Players access the experience via QR code or direct URL, making the entry barrier as low as a Google search.
Location-bound puzzles
Each puzzle is designed specifically for its physical location. Players must observe, explore and interact with their real environment to progress — the space itself is the game board.
Multilingual support
Tracks can be delivered in multiple languages simultaneously. Players select their preferred language at the start. The base subscription includes one language; additional languages are available as add-ons.
AR integration
Epic Scape supports augmented reality elements within the experience, adding visual layers to real-world objects and locations without requiring a separate app.
AI-assisted content
Epic Scape uses AI to generate dynamic player-facing responses and content within experiences. This reduces manual content authoring and enables faster deployment of new tracks.
Real-time data dashboard
Every player session generates anonymous behavioural data. Venues access it through a custom-built dashboard powered by Supabase, updating in real time. The dashboard is exportable as PDF and CSV.
Cookie-based memory layer (in development)
A planned feature that will recognise returning players across multiple Epic Scape locations via browser cookies, enabling measurement of place memorability over time without collecting personal data.
...
Six sub-puzzles plus a final, designed as an hour-length experience. Epic Scape also offers extended tracks tailored to a venue's specific space, story and audience — built on request.
Each track includes:

A narrative framework and script written by Epic Scape's game writer
...
Browser-based experience engine
Real-time data dashboard (Supabase-powered, PDF/CSV export)
Multilingual support
AR element integration
